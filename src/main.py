...
async def load_assets():
    ...
    Tutorial_Images = {
        "0": load_image("empty_tutorial.png"),
        "1": load_image("tutorial_1.png"),
        "2": load_image("tutorial_2.png"),
        "3": load_image("tutorial_3.png"),
        "4": load_image("tutorial_4.png"),
        "end": load_image("tutorial_end.png"),
        "1.1": load_image("1_tutorial__screenshot.png"),
        "1.2": load_image("3_tutorial__screenshot.png"),
        "1.3": load_image("5_tutorial__screenshot.png"),
        "2.1": load_image("flagtutorialscreenshot.png"),
        "3.1": load_image("hidden1.png"),
        "3.2": load_image("hidden2.png"),
        "4.1": load_image("hint_tutorialscreenshot.png"),
    }
    ...
    return Title_sequence,Tutorial_Images,Images

Title_sequence, Tutorial_Images, Images = asyncio.run(load_assets())

# region Global Gameplay Variables -------------------------------------------------------------------------------------\
#Chance of hidden square is 1/num
Hidden_num = 6
...
tutorial_button_1 = None
tutorial_button_2 = None
...
def paint() -> None:
    ...
    global Title, Tutorial, Pause,Wrong

    if Title:
        if Tutorial == 9 or Tutorial == 0:
            draw_play_button()
        else:
            play_button_reset()
    ...
    draw_squares()
    if Pause:
        draw_rect_center(constants.window, (constants.BOARD_CENTER_X, constants.BOARD_CENTER_Y),(400,200),constants.COLOR_BLUE)
        draw_text(constants.window, f"Number of Flags: {NUM_MINES - len(flags)}", 20, constants.COLOR_WHITE, (constants.BOARD_CENTER_X, constants.BOARD_CENTER_Y))
        draw_text(constants.window, f"Hints Left: {numHintsleft}", 20, constants.COLOR_WHITE, (constants.BOARD_CENTER_X, constants.BOARD_CENTER_Y+30))
        draw_text(constants.window, f" Elapsed Time: {str(seconds_elapsed)}", 20, constants.COLOR_WHITE,(constants.BOARD_CENTER_X, constants.BOARD_CENTER_Y-30))
    ...
    if Wrong !=0:
        draw_button(constants.window, "Wrong!", (int(constants.WINDOW_WIDTH/2), int(constants.WINDOW_HEIGHT/2)), 40,constants.COLOR_BLACK, constants.COLOR_BLUE)
        Wrong -=1


def draw_title() -> None:
    global Title, Opening, tick, frame_index,GameSet, NUM_ROWS,NUM_COLS,NUM_MINES, mines_Selection, width_Selection, height_Selection, GameSet, Tutorial, Title_sequence
    FRAME_DELAY = 2
    if tick < 15 * FRAME_DELAY:
        ...
        if Tutorial == 1:
            draw_image(constants.window, Tutorial_Images["1"], (constants.BOARD_CENTER_X, constants.BOARD_CENTER_Y))
        elif Tutorial == 2:
            draw_image(constants.window, Tutorial_Images["0"], (constants.BOARD_CENTER_X, constants.BOARD_CENTER_Y))
            draw_image(constants.window, Tutorial_Images["1.1"], (200, 200))
            draw_image(constants.window, Tutorial_Images["1.2"], (500, 200))
            draw_image(constants.window, Tutorial_Images["1.3"], (800, 200))
            draw_button(constants.window, "A '1' square ^", (200, 350), 30, constants.COLOR_BLACK, constants.COLOR_BLUE,(10, 10))
            draw_button(constants.window, "A '3' square ^", (500, 350), 30, constants.COLOR_BLACK, constants.COLOR_BLUE,(10, 10))
            draw_button(constants.window, "A '5' square ^", (800, 350), 30, constants.COLOR_BLACK, constants.COLOR_BLUE,(10, 10))
        elif Tutorial == 3:
            draw_image(constants.window, Tutorial_Images["2"], (constants.BOARD_CENTER_X, constants.BOARD_CENTER_Y))
        elif Tutorial == 4:
            draw_image(constants.window, Tutorial_Images["0"], (constants.BOARD_CENTER_X, constants.BOARD_CENTER_Y))
            draw_image(constants.window, Tutorial_Images["2.1"], (200, 200))
            draw_button(constants.window, "<--- These are flags, preventing deaths", (650, 200), 30, constants.COLOR_BLACK, constants.COLOR_BLUE,(10, 10))
        elif Tutorial == 5:
            draw_image(constants.window, Tutorial_Images["3"], (constants.BOARD_CENTER_X, constants.BOARD_CENTER_Y))
        elif Tutorial == 6:
            draw_image(constants.window, Tutorial_Images["0"], (constants.BOARD_CENTER_X, constants.BOARD_CENTER_Y))
            draw_image(constants.window, Tutorial_Images["3.1"], (200, 200),0,2)
            draw_image(constants.window, Tutorial_Images["3.2"], (800, 200),0,2)
            draw_button(constants.window, "< A hidden square being solved >", (500, 200), 25,constants.COLOR_BLACK, constants.COLOR_BLUE, (10, 10))
        elif Tutorial == 7:
            draw_image(constants.window, Tutorial_Images["4"], (constants.BOARD_CENTER_X, constants.BOARD_CENTER_Y))
        elif Tutorial == 8:
            draw_image(constants.window, Tutorial_Images["0"], (constants.BOARD_CENTER_X, constants.BOARD_CENTER_Y))
            draw_image(constants.window, Tutorial_Images["4.1"], (200, 200))
            draw_button(constants.window, "<--- A hint being used to reveal mines", (650, 200), 30,constants.COLOR_BLACK, constants.COLOR_BLUE, (10, 10))
        elif Tutorial == 9:
            draw_image(constants.window, Tutorial_Images["end"], (constants.BOARD_CENTER_X, constants.BOARD_CENTER_Y))
    else:
        Opening = False
...
def create_normal_squares(x , y) -> None:
    global player_move_input, first_click , board_origin_y, board_origin_x, Wrong, Hidden_num
    ...
    hints.clear()
    rand_val = random.randint(1, Hidden_num)
    if rand_val == 1 and mine_count > 0 and game_manager.board.game_board[row][col] != 2 and not has_hidden_square_been_triggered:
...
def draw_play_button() -> None:
    global play_button, GameSet, Tutorial

    if Tutorial == 9:
        play_button = draw_button(constants.window, "xxxx",(591, 486), 40, constants.COLOR_RED, constants.COLOR_GREEN,(53,20))
    elif Tutorial !=0:
        return
    elif GameSet:
...
def draw_tutorial_button():
    global tutorial_button_1, tutorial_button_2, Tutorial

    tutorial_button_1 = None
    tutorial_button_2 = None
    if Tutorial == 0:
        tutorial_button_2 = draw_button(constants.window, "xxxx",(826, 385), 40, constants.COLOR_RED, constants.COLOR_GREEN,(117,30))
    elif Tutorial == 9:
        tutorial_button_1 = draw_button(constants.window, "xxxx", (361, 486), 40, constants.COLOR_RED,constants.COLOR_GREEN, (53, 20))
        tutorial_button_2 = None
    elif Tutorial != 0:
        tutorial_button_1 = draw_button(constants.window, "xxxx",(361, 486), 40, constants.COLOR_RED, constants.COLOR_GREEN,(53,20))
        tutorial_button_2 = draw_button(constants.window, "xxxx",(591, 486), 40, constants.COLOR_RED, constants.COLOR_GREEN,(53,20))
...
def process_mouse_event(event: pygame.event.Event) -> None:
    ...
    global player_move_input
    global reset_button, play_button, mines_selection_button, width_selection_button, height_selection_button, mines_Selection, width_Selection, height_Selection, tutorial_button_1, tutorial_button_2
    global Title, GameSet, Opening, Tutorial
        ...
        if Title:
            if GameSet:
                if play_button is not None and play_button.collidepoint(event.pos):
                    ...
                    Title = False
                    startgame()
                    play_sfx("menu_select_sound.ogg")
                ...
                GameSet = True
                Tutorial = 0
            elif tutorial_button_1 is not None and tutorial_button_1.collidepoint(event.pos):
                Tutorial -= 1
                play_sfx("menu_select_sound.ogg")
            elif tutorial_button_2 is not None and tutorial_button_2.collidepoint(event.pos):
                Tutorial += 1
                play_sfx("menu_select_sound.ogg")
...
def process_key_event(event: pygame.event.Event) -> None:
    global Title, GameSet, Opening, tick, frame_index, clock,Pause
...
def reset() -> None:
    global have_mines_been_placed, mines, squares, flags, first_click, Title, GameSet, Opening, tick, frame_index, clock, hints, hidden_squares, seconds_elapsed, numHintsleft, Tutorial, Pause
    have_mines_been_placed = False
    mines = []
    squares = []
    flags = []
...
async def play_game():
    ...
    run = True
    play_music("background_audio.ogg", 1.25)
    while run:
